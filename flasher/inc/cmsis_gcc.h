...
__STATIC_FORCEINLINE uint32_t __get_AIRCR(void)
{
  uint32_t result;

  __ASM volatile ("MRS %0, aircr" : "=r" (result) );
  return(result);
}
...
__STATIC_FORCEINLINE void __set_AIRCR(uint32_t aircr)
{
  __ASM volatile ("MSR aircr, %0" : : "r" (aircr) : "memory");
}
